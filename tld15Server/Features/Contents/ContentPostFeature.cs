...
/// <summary>
/// Stores a content whole — the picture and the links it carries, and the body of every locale —
/// replacing what was there. The content and all of its translations are written on the same save, so
/// their dates and versions stay in step and the version of the content alone says whether a caller
/// holds a stale copy.
/// </summary>
/// <remarks>
        ...
        ) : ICommandHandler<Command, Result>
    {
        public async ValueTask<Result> Handle(Command cmd, CancellationToken ctn)
        {
            ...
            await using (var contextBusiness = await contextBusinessFactory.CreateDbContextAsync(ctn))
            {
                ...
                var written = new HashSet<string>(markdown.Keys, StringComparer.Ordinal);
                ...
                foreach (var translation in content.Translations)
                {
                    ...
                    // whose text did not change, so the whole content moves as one.
                    translation.VersionLocal++;
                }

                content.PosterUrl = posterUrl;
                content.LinksJson = SharedLink.DictionaryToJson(links);
        ...
        /// <summary>
        /// The links of the content as one dictionary, keyed by the address each one opens. A row
